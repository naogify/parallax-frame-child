 ...
 * Add breadcrumb.
 *
 * @action parallax_frame_after_header
 *
 * @since Parallax Frame 0.1
 */
if ( !function_exists( 'parallax_frame_add_breadcrumb' ) ) :
	function parallax_frame_add_breadcrumb() {
		...
		if ( $options['breadcumb_option'] ){
			...
			echo parallax_frame_custom_breadcrumbs( $options['breadcumb_on_homepage'], $delimiter );
		}
	}
endif;
add_action( 'parallax_frame_before_content', 'parallax_frame_add_breadcrumb', 50 );
 ...
 * @since Parallax Frame 0.1
 */
if ( !function_exists( 'parallax_frame_custom_breadcrumbs' ) ) :

	function parallax_frame_custom_breadcrumbs( $showOnHome, $delimiter ) {

		/* === OPTIONS === */
		$text['home']     = esc_html__( 'Home', 'parallax-frame' ); // text for the 'Home' link
		$text['category'] = esc_html__( '%1$s Archive for %2$s', 'parallax-frame' ); // text for a category page
		$text['search']   = esc_html__( '%1$sSearch results for: %2$s', 'parallax-frame' ); // text for a search results page
		$text['tag']      = esc_html__( '%1$sPosts tagged %2$s', 'parallax-frame' ); // text for a tag page
		$text['author']   = esc_html__( '%1$sView all posts by %2$s', 'parallax-frame' ); // text for an author page
		$text['404']      = esc_html__( 'Error 404', 'parallax-frame' ); // text for the 404 page
		...
		if ( is_front_page() ) {
			...
			elseif ( is_day() ) {
				echo sprintf( $link, esc_url( get_year_link( get_the_time( __( 'Y', 'parallax-frame' ) ) ) ), get_the_time( __( 'Y', 'parallax-frame' ) ) ) ;
				echo sprintf( $link, esc_url( get_month_link( get_the_time( __( 'Y', 'parallax-frame' ) ), get_the_time( __( 'm', 'parallax-frame' ) ) ) ), get_the_time( __( 'F', 'parallax-frame' ) ) );
				echo $before . get_the_time( __( 'd', 'parallax-frame' ) ) . $after;

			}
			elseif ( is_month() ) {
				echo sprintf( $link, esc_url( get_year_link( get_the_time( __( 'Y', 'parallax-frame' ) ) ) ), get_the_time( __( 'Y', 'parallax-frame' ) ) ) ;
				echo $before . get_the_time( __( 'F', 'parallax-frame' ) ) . $after;

			}
			elseif ( is_year() ) {
				echo $before . get_the_time( __( 'Y', 'parallax-frame' ) ) . $after;
			...
			if ( get_query_var( 'paged' ) ) {
				...
				echo sprintf( esc_html__( 'Page %s', 'parallax-frame' ), max( $paged, $page ) );
				if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) {
					echo ')';
				}
			}
	...
	} // end parallax_frame_breadcrumb_lists
endif;
