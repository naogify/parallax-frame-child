 ...
 * @since Parallax Frame 0.1
 */
class parallax_frame_featured_post_widget extends WP_Widget {
	...
	function __construct() {
		...
		$this->defaults = array(
			...
			'content_type'    => '',
			'content_limit'   => 200,
			'more_text'       => esc_html__( 'Read More ...', 'parallax-frame' ),
		);

		$widget_ops = array(
			'classname'   => 'ct-featured-post ctfeaturedpostpageimage',
			'description' => esc_html__( 'Displays featured posts with thumbnails', 'parallax-frame' ),
		);
		...
		parent::__construct(
			...
			__( 'CT: Featured Posts', 'parallax-frame' ), // Name
			$widget_ops,
			$control_ops
		);
	}
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'post_type' ); ?>"><?php esc_html_e( 'Select Posts From', 'parallax-frame' ); ?>:</label>
			<select class="ct_feat_post_post_type widefat" id="<?php echo $this->get_field_id( 'post_type' ); ?>" name="<?php echo $this->get_field_name( 'post_type' ); ?>">
				<?php
					$post_type_choices = array(
						'all-categories'  => esc_html__( 'All Categories', 'parallax-frame' ),
						'select-category' => esc_html__( 'Select Category', 'parallax-frame' ),
						'id'              => esc_html__( 'ID', 'parallax-frame' ),
					);
		...
		<p <?php echo ( 'id' == $instance['post_type'] )? '' : $style;  ?>>
			<label for="<?php echo $this->get_field_id( 'post_id' ); ?>"><?php esc_html_e( 'Post ID', 'parallax-frame' ); ?>:</label>
			...
			<span class="description"><?php esc_html_e( 'Separate Multiple Post IDs by ,(comma)', 'parallax-frame' ); ?></span>
		</p>

        <p <?php echo ( 'select-category' == $instance['post_type'] )? '': $style; ?>>
        	<?php
        		parallax_frame_dropdown_categories(
        ...
        <p <?php echo ( 'id' == $instance['post_type'] )? $style : '';  ?>>
			<label for="<?php echo $this->get_field_id( 'post_number' ); ?>"><?php esc_html_e( 'No. of Posts', 'parallax-frame' ); ?>:</label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'orderby' ); ?>"><?php esc_html_e( 'Order By', 'parallax-frame' ); ?>:</label>
			<select id="<?php echo $this->get_field_id( 'orderby' ); ?>" name="<?php echo $this->get_field_name( 'orderby' ); ?>" class="widefat">
				<?php
					$choices = array(
						'date'   => esc_html__( 'Date', 'parallax-frame' ),
						'ID'   => esc_html__( 'ID', 'parallax-frame' ),
						'author'   => esc_html__( 'Author', 'parallax-frame' ),
						'title'   => esc_html__( 'title', 'parallax-frame' ),
						'name'   => esc_html__( 'Name', 'parallax-frame' ),
						'comment_count'   => esc_html__( 'Comment Count', 'parallax-frame' ),
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'order' ); ?>"><?php esc_html_e( 'Order', 'parallax-frame' ); ?>:</label>
			<select id="<?php echo $this->get_field_id( 'order' ); ?>" name="<?php echo $this->get_field_name( 'order' ); ?>" class="widefat">
				<?php
					$choices = array(
						'DESC'   => esc_html__( 'Descending (3, 2, 1)', 'parallax-frame' ),
						'ASC'   => esc_html__( 'Ascending (1, 2, 3)', 'parallax-frame' ),
					);
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'layout' ); ?>"><?php esc_html_e( 'Layout', 'parallax-frame' ); ?>:</label>
			<select id="<?php echo $this->get_field_id( 'layout' ); ?>" name="<?php echo $this->get_field_name( 'layout' ); ?>" class="widefat">
				<?php
					$post_type_choices = array(
						'one-column'   => esc_html__( '1 Column', 'parallax-frame' ),
						'two-column'   => esc_html__( '2 Column', 'parallax-frame' ),
						'three-column' => esc_html__( '3 Column', 'parallax-frame' ),
						'four-column'  => esc_html__( '4 Column', 'parallax-frame' ),
					);
		...
		<p>
        	...
        	<label for="<?php echo $this->get_field_id('disable_image'); ?>"><?php esc_html_e( 'Check to Disable Image', 'parallax-frame' ); ?></label><br />
        </p>

        <p <?php echo ( $instance['disable_image'] )? $style : '';  ?>>
			<label for="<?php echo $this->get_field_id( 'image_alignment' ); ?>"><?php esc_html_e( 'Image Alignment', 'parallax-frame' ); ?>:</label>
			<select id="<?php echo $this->get_field_id( 'image_alignment' ); ?>" name="<?php echo $this->get_field_name( 'image_alignment' ); ?>" class="widefat">
				<?php
					$image_alignment_choices = array(
						'top'    => esc_html__( 'Top', 'parallax-frame' ),
						'left'   => esc_html__( 'Left', 'parallax-frame' ),
						'right'  => esc_html__( 'Right', 'parallax-frame' ),
						'center' => esc_html__( 'Center', 'parallax-frame' ),
					);
		...
		<p <?php echo ( $instance['disable_image'] )? $style : '';  ?>>
			<label for="<?php echo $this->get_field_id( 'image_size' ); ?>"><?php esc_html_e( 'Image Size', 'parallax-frame' ); ?>:</label>
			<select id="<?php echo $this->get_field_id( 'image_size' ); ?>" class="parallax-frame-image-size-selector" name="<?php echo $this->get_field_name( 'image_size' ); ?>">
				<option value="thumbnail">thumbnail (<?php echo get_option( 'thumbnail_size_w' ); ?>x<?php echo get_option( 'thumbnail_size_h' ); ?>)</option>
				<?php
				$sizes = parallax_frame_get_additional_image_sizes();
        ...
        <p>
        	...
        	<label for="<?php echo $this->get_field_id('disable_title'); ?>"><?php esc_html_e( 'Check to Disable Title', 'parallax-frame' ); ?></label><br />
        </p>

        <p>
			<span class="description"><?php esc_html_e( 'Post Meta Info', 'parallax-frame' ); ?></span><br/>
        	...
        	<label for="<?php echo $this->get_field_id('hide_category'); ?>"><?php esc_html_e( 'Check to Hide Cagegory', 'parallax-frame' ); ?></label><br />
        	...
        	<label for="<?php echo $this->get_field_id('hide_tags'); ?>"><?php esc_html_e( 'Check to Hide Tags', 'parallax-frame' ); ?></label><br />
        	...
        	<label for="<?php echo $this->get_field_id('hide_posted_on'); ?>"><?php esc_html_e( 'Check to Hide Posted On Date', 'parallax-frame' ); ?></label><br />
        	...
        	<label for="<?php echo $this->get_field_id('hide_author'); ?>"><?php esc_html_e( 'Check Hide to Author', 'parallax-frame' ); ?></label><br />
		</p>

        <p>
			<label for="<?php echo $this->get_field_id( 'content_type' ); ?>"><?php esc_html_e( 'Content Type', 'parallax-frame' ); ?>:</label>
			<select class="ct_feat_post_content_type" id="<?php echo $this->get_field_id( 'content_type' ); ?>" name="<?php echo $this->get_field_name( 'content_type' ); ?>">
				<option value="content" <?php selected( 'content', $instance['content_type'] ); ?>><?php esc_html_e( 'Show Content', 'parallax-frame' ); ?></option>
				<option value="excerpt" <?php selected( 'excerpt', $instance['content_type'] ); ?>><?php esc_html_e( 'Show Excerpt', 'parallax-frame' ); ?></option>
				<option value="content-limit" <?php selected( 'content-limit', $instance['content_type'] ); ?>><?php esc_html_e( 'Show Content Limit', 'parallax-frame' ); ?></option>
				<option value="" <?php selected( '', $instance['content_type'] ); ?>><?php esc_html_e( 'No Content', 'parallax-frame' ); ?></option>
			</select>
			<br />
			<label <?php echo ( 'content-limit' != $instance['content_type'] )? $style : '';  ?>for="<?php echo $this->get_field_id( 'content_limit' ); ?>"><?php esc_html_e( 'Limit content to', 'parallax-frame' ); ?>
				...
				<?php esc_html_e( 'characters', 'parallax-frame' ); ?>
			</label>
		</p>

		<p>
			<label for="<?php echo $this->get_field_id( 'more_text' ); ?>"><?php esc_html_e( 'More Text (if applicable)', 'parallax-frame' ); ?>:</label>
	...
	function update( $new_instance, $old_instance ) {
		...
		$instance['disable_image']   = parallax_frame_sanitize_checkbox( $new_instance['disable_image'] );
		...
		$instance['disable_title']   = parallax_frame_sanitize_checkbox( $new_instance['disable_title'] );
		$instance['hide_category']   = parallax_frame_sanitize_checkbox( $new_instance['hide_category'] );
		$instance['hide_tags']       = parallax_frame_sanitize_checkbox( $new_instance['hide_tags'] );
		$instance['hide_posted_on']  = parallax_frame_sanitize_checkbox( $new_instance['hide_posted_on'] );
		$instance['hide_author']     = parallax_frame_sanitize_checkbox( $new_instance['hide_author'] );
	...
	function widget( $args, $instance ) {
		...
		if ( $loop->have_posts() ) {
			...
			<article class="' . esc_attr( $class ) . '">';
				...
				$output .= parallax_frame_get_meta(
							$instance['hide_category'],
							$instance['hide_tags'],
							$instance['hide_posted_on'],
							$instance['hide_author']
				...
				elseif ( 'content-limit' == $instance['content_type'] ) {
					$output .= '<div class="entry-summary">';
					$output .= parallax_frame_get_the_content_limit( (int) $instance['content_limit'], esc_html( $instance['more_text'] ) );
					$output .= '</div><!-- .entry-summary -->';
				}
 ...
 * Initialize Widget
 */
function parallax_frame_featured_post_init() {
	register_widget( 'parallax_frame_featured_post_widget' );
}
add_action( 'widgets_init', 'parallax_frame_featured_post_init' );
